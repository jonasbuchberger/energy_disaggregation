...
class CNN1D(nn.Module):
    ...
    def __init__(self, num_classes, window_size, n_quantiles, dropout=0.1, pooling_size=16):
        """
        ...
        Args:
            ...
            n_quantiles (int) : number of quantiles used
        ...
        self.mlp3 = nn.Linear(1024, self.num_classes * self.window_size * self.n_quantiles)
    ...
    def forward(self, x):
        ...
        states = self.mlp2(x).reshape(x.size(0), self.window_size, self.num_classes)
        power = self.mlp3(x).reshape(x.size(0), self.window_size, self.n_quantiles, self.num_classes)
